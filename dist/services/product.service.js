"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProductService = void 0;
const product_1 = __importDefault(require("../model/product"));
// Services
class ProductService {
    static getProduct() {
        return __awaiter(this, void 0, void 0, function* () {
            const product = yield product_1.default.find({});
            return product;
        });
    }
    static findProductById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const product = yield product_1.default.findById(id);
            return product;
        });
    }
    static createProduct(newProduct) {
        return __awaiter(this, void 0, void 0, function* () {
            const product = yield product_1.default.create(newProduct);
            return product;
        });
    }
    static updateProductById(id, newProduct) {
        return __awaiter(this, void 0, void 0, function* () {
            const product = yield product_1.default.findByIdAndUpdate(id, newProduct);
            return product;
        });
    }
    static deleteProduct(id, newProduct) {
        return __awaiter(this, void 0, void 0, function* () {
            const product = yield product_1.default.findByIdAndDelete(id, newProduct);
            return product;
        });
    }
    static mapProductResDto(model) {
        return {
            productId: model._id,
            pictureUrl: model.pictureUrl,
            userId: model.userId,
            displayName: model.displayName,
            statusMessage: model.statusMessage,
            prod_img: model.prod_img,
            prod_name: model.prod_name,
            prod_desc: model.prod_desc,
            prod_price: model.prod_price,
            updated_at: model.update_at,
        };
    }
}
exports.ProductService = ProductService;
